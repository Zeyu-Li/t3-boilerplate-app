...
- Run `yarn` or `npm i` to install dependency (note that @type/\<lib> sometimes still has trouble installing so you might need to install that manually ie `yarn add @types@react`)
- run prisma init `npx prisma db push`
